...
import math
import time
import datetime as dt  # (ADAPT) suporte a GMT-3 para reporte/headless

# ======================================================================
# CONTADOR DE PESSOAS - VERSÃO 1 - UI ENTRADA/SAÍDA
# ======================================================================
...
IOU_WEIGHT = 0.55
LOW_IOU_PENALTY_THRESH = 0.08          # NOVO: limiar de IoU para penalidade leve
LOW_IOU_PENALTY_FACTOR = 1.15          # NOVO: fator de penalidade leve (25%)
...
# ---------- RTSP / Modelo ----------
# (ADAPT) RTSP_URL agora pode vir do ambiente; mantém default original
...
# (ADAPT) Headless flags e duração máxima (~1h para Actions)
HEADLESS = os.getenv("HEADLESS", "0") == "1"
HEADLESS_MAX_SEC = int(os.getenv("HEADLESS_MAX_SEC", "3600"))  # segundos (~1h)
...
def _within_run_window_gmt3(now=None):
    if now is None:
        now = _now_gmt3()
    # roda e reporta entre 09–21 (GMT-3)
...
class PersonTrack:
    def __init__(self, track_id, detection_box, initial_side, quick_birth=False):
        ...
        # NOVO: armazena tipo de cruzamento
        self.last_crossing_type = None  # "ENTRADA" ou "SAÍDA"

        # NOVO: flags para bloquear contagem duplicada do mesmo ID
...
class PeopleCounter:
        ...
        for i, tid in enumerate(track_ids):
            ...
            for j, det in enumerate(detections):
                ...
                if distance < MAX_TRACKING_DISTANCE:
                    ...
                    # NOVO: penalidade leve quando IoU é muito baixo (reduz swaps)
                    ...
                    if initial_sign != current_sign and current_sign != 0:
                        ...
                        if t.crossing_confirmations >= needed:
                            ...
                            if initial_sign == self.entry_sign:
                                direction = "ENTRADA"
                                # NOVO: bloqueia segunda contagem de ENTRADA para o mesmo ID
                                if t.has_entrada:
                                    # já contou entrada antes; apenas atualiza estado e segue
                                ...
                                direction = "SAÍDA"
                                # NOVO: bloqueia segunda contagem de SAÍDA para o mesmo ID
                                if t.has_saida:
                                    t.last_cross_frame = self.frame_count
                                    t.initial_side = current_side
                                    t.crossing_confirmations = 0
                            ...
                            # Contagem total (mantida)
                            t.total_crosses += 1
                            t.last_cross_frame = self.frame_count
                            self.total_crossings += 1
                            t.last_crossing_type = direction

                            # Reseta estado pós-contagem (mantido)
                            t.initial_side = current_side
                            t.crossing_confirmations = 0
                            t.first_in_gate_frame = None
    ...
    def draw_interface(self, frame):
        ...
        # Painel com Entrada/Saída separados
        ...
        cv2.putText(frame, f"Confirm: {CROSSING_CONFIRMATION_FRAMES} | Gate: {self.gate_width}px | CD: {COOLDOWN_AFTER_CROSS}",
                    (px + 10, py + 120), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (200, 200, 200), 1)
        cv2.putText(frame, f"Entrada: {'RIGHT' if self.entry_sign > 0 else 'LEFT'} | Conf: {CONFIDENCE_THRESHOLD:.2f} | MinTrack: {MIN_TRACK_LENGTH}",
                    (px + 10, py + 140), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (200, 200, 200), 1)
        ...
        for t in self.tracks.values():
            ...
            dist = math.hypot(center[0] - bbox_center[0], center[1] - bbox_center[1])
            if dist > CENTER_BBOX_ALERT_DISTANCE:
                cv2.line(frame, center, bbox_center, (0, 165, 255), 2)
            ...
            if t.quick_birth:
                label += " QB"
            if t.is_anomalous:
                label += f" !{t.center_bbox_anomaly_count}"

            cv2.putText(frame, label, (x, y - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)

            if len(t.center_history) > 1:
                pts = list(t.center_history)[-10:]
                for i in range(1, len(pts)):
                    cv2.line(frame, pts[i - 1], pts[i], color, 1)

            y_info = y + h + 14
            if t.in_gate_area:
                cv2.putText(frame, f"GATE {t.crossing_confirmations}/{CROSSING_CONFIRMATION_FRAMES}",
                            (x, y_info), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
                y_info += 13

                if t.first_in_gate_frame is not None:
                    time_in_gate = self.frame_count - t.first_in_gate_frame
                    remaining = GATE_TIMEOUT - time_in_gate
                    if remaining < 10:
                        cv2.putText(frame, f"TO:{max(0, remaining)}", (x, y_info),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 100, 0), 1)

            if not t.can_cross_again(self.frame_count):
                cd_remaining = COOLDOWN_AFTER_CROSS - (self.frame_count - t.last_cross_frame)
                cv2.putText(frame, f"CD:{cd_remaining}", (x, y_info),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 150, 0), 1)

        instructions = "Q:sair | R:reset | SPACE:pause | +/-:conf | ]/[:confirm | {/}:cooldown | </>:gate"
    ...
    def run(self):
        ...
        print("=" * 60)
        print("CONTADOR DE PESSOAS v2.2 - ENTRADA/SAÍDA")
        print("=" * 60)
        print("MELHORIAS:")
        print("  ✓ Gate maior, visível e ajustável em tempo real (</>)")
        print("  ✓ Quick-birth ampliado na direita + janela de graça maior")
        print("  ✓ Associação/kalman mais tolerantes (ID não fica para trás)")
        print("  ✓ Confirmação 1 frame (entrada direita conta a tempo)")
        print("  ✓ Contadores separados: ENTRADA (verde) e SAÍDA (vermelho)")
        print("  ✓ Flags anti-duplicação por ID (has_entrada / has_saida)")
        print("  ✓ Penalidade leve quando IoU<0.08 para reduzir swap de IDs")
        print("=" * 60)

        paused = False
        last_frame = None

        start_ts = time.time()  # (ADAPT) início da janela de execução headless

        while True:
            # (ADAPT) encerra no modo headless após HEADLESS_MAX_SEC (~1h)
            if HEADLESS and (time.time() - start_ts) >= HEADLESS_MAX_SEC:
                break
            ...
            if not HEADLESS:
                cv2.imshow("Contador de Pessoas v2.2", frame)
            ...
            if not HEADLESS:
                ...
                if key == ord('q'):
                    ...
                    CONFIDENCE_THRESHOLD = min(0.9, round(CONFIDENCE_THRESHOLD + 0.02, 2))
                    print(f"Confiança ajustada: {CONFIDENCE_THRESHOLD:.2f}")
                elif key == ord('-'):
                    CONFIDENCE_THRESHOLD = max(0.05, round(CONFIDENCE_THRESHOLD - 0.02, 2))
                    print(f"Confiança ajustada: {CONFIDENCE_THRESHOLD:.2f}")
                elif key == ord(']'):
                    CROSSING_CONFIRMATION_FRAMES = max(1, CROSSING_CONFIRMATION_FRAMES - 1)
                    print(f"Confirmações ajustadas: {CROSSING_CONFIRMATION_FRAMES}")
                elif key == ord('['):
                    CROSSING_CONFIRMATION_FRAMES = min(8, CROSSING_CONFIRMATION_FRAMES + 1)
                    print(f"Confirmações ajustadas: {CROSSING_CONFIRMATION_FRAMES}")
                elif key == ord('}'):
                    COOLDOWN_AFTER_CROSS = max(3, COOLDOWN_AFTER_CROSS - 2)
                    print(f"Cooldown ajustado: {COOLDOWN_AFTER_CROSS} frames")
                elif key == ord('{'):
                    COOLDOWN_AFTER_CROSS = min(30, COOLDOWN_AFTER_CROSS + 2)
                    print(f"Cooldown ajustado: {COOLDOWN_AFTER_CROSS} frames")
                elif key == ord('>'):
                    self.gate_width = min(240, self.gate_width + 10)
                    print(f"Gate width: {self.gate_width}px")
                elif key == ord('<'):
                    self.gate_width = max(40, self.gate_width - 10)
                    print(f"Gate width: {self.gate_width}px")
            else:
                # headless: não lê teclado
                pass
        ...
        # ================== Reporter (Excel Online / CSV) ==================
        try:
            from reporter import Reporter
            now_gmt3 = _now_gmt3()
            if _within_run_window_gmt3(now_gmt3):
                rep = Reporter()
                # Envia apenas ENTRADAS (conforme necessidade de planilha)
                rep.write_hourly(self.entrada_count, when=now_gmt3)
                print(f"✓ Reporte enviado ({now_gmt3.strftime('%Y-%m-%d %H:%M')} GMT-3): ENTRADAS={self.entrada_count}")
            else:
                print("⏭ Fora da janela 09–21 (GMT-3); não reportado.")
        except Exception as e:
            print(f"[AVISO] Falha ao reportar: {e}")
        # ==================================================================
        ...
        print(f"Anomalias detectadas: {self.anomaly_detections}")
        print("=" * 60)
